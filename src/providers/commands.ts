...
function useCommands(server: Server) {
  ...
  async function poweroff() {
    const res =
      await $`sshpass -p '${server.password}' ssh -o ConnectTimeout=2 -o StrictHostKeyChecking=no -o StrictHostKeyChecking=accept-new ${server.user}@${server.ip} "poweroff" | printf ''`.text(
        "combined",
      );
